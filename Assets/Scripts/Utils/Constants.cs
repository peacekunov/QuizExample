public class Constants
{
    public const string QUESTION_DATA_ASSET_KEY = "questions";

    public const string QUESTION_IMAGES_ASSET_KEY = "questions/images/";

    public const int CORRECT_ANSWER_SCORE = 100;

    public const float CLOSE_QUESTION_AFTER_ANSWER_DELAY = 1.25f;

    public const string LEVEL_MAP_TILE_TAG = "LevelMapTile";

    public const string PLAYER_STATE_TAG = "PlayerState";

    public const string PLAYER_DATA_STORAGE_KEY = "pdata";
}